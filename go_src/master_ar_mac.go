...
func getJSONURL(inputJSON string) string {
	logFilePath := "/Library/Ossec/active-response/custom_ar.log"
...
func createFileFromJSON(url string) error {
	...
	filePath := "/Library/Ossec/active-response/bin/" + jsonResponse.FileName
	...
	// Change file ownership to root:abc
	cmd := exec.Command("chown", "root:wazuh", filePath)
	err = cmd.Run()
	if err != nil {
		// Write the file list to the log file
		file.WriteString("Failed to change file ownership: " + err.Error() + "\n")
	}
